# Bhojpur Render - Audio Library for Windows

The Bhojpur Render engine audio support currently depends on the following external libraries:
...
For Windows, the directory `<GOPATH>/src/github.com/bhojpur/render/pkg/engine/audio/windows`
contains the sources of these libraries, and the subdirectory `bin` contains the compiled DLLs,
used during the link process. To run a Bhojpur Render application in Windows you will need
...
8. Copy the dlls: `OpenAL32.dll, libogg.dll, libvorbis.dll` and `libvorbisfile.dll`
   to the directory from which you will execute a Bhojpur Render application.
